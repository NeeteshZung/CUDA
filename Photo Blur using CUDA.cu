...
//nvcc lodepng.cpp Q.N.4.cu -o Q.N.4
//./Nitesh Bishwakarma (Photo Blur using CUDA)
...
{
		char widthVal = *width;
    unsigned int i = (row * widthVal * 4) + (col * 4);
    return image[i];
}
...
{
		char widthVal = *width;
    unsigned int i = (row * widthVal * 4) + (col * 4) + 1;
    return image[i];
}
...
{
		char widthVal = *width;
    unsigned int i = (row * widthVal * 4) + (col * 4) + 2;
    return image[i];
}
...
{
		char widthVal = *width;
    unsigned int i = (row * widthVal * 4) + (col * 4) + 3;
    return image[i];
}
...
__global__ void applyBlurFilter(unsigned int *height, unsigned int *width, unsigned int *result, unsigned char * newImage, unsigned char * image){
...
int main(void){
	...
	const char* filename = "Zac.png";
	const char* newFileName = "blureZac.png";
  ...
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
	applyBlurFilter<<<height, width>>>(gpuHeight, gpuWidth, gpuResult, d_out, d_in);
